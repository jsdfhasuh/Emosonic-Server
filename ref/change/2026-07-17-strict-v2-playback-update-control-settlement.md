...
状态：**阶段 0—1 已完成：strict-v2 `2.4.0` r10 契约、ADR-0022 和服务端更动说明已更新；等待
服务端工程师评审。尚未修改 manifest、fixtures、服务端实现或 Flutter 代码。**
...
9. 每个 pending 远程命令默认最多等待 15 秒；超时必须失败，不能永久 pending。
...
- `queue.playItem`、`player.next` 或 `player.prev` 失败时，所有更高版本且仍为 `pending` 的事务也进入
  `failed`，`errorCode:"dependency_failed"`；
- pending 超过默认 15000ms 执行期限后进入 `failed`，`errorCode:"execution_timeout"`；
- authority 断线、连接被替换或服务端重启造成结果不明时进入 `failed`，
  `errorCode:"execution_unknown"`，不得重连重发；
...
- 每条 pending 记录保存 `acceptedAtMs` 和 `deadlineAtMs`；部署可以调整期限，但不能关闭超时结算。
...
契约阶段还必须固定“accepted control target”和“applied device state”的写入时点。允许主 Context 在
...
事务标记为 failed/dependency_failed。failed canonical update 中的 `controlVersion` 使用当前最高
canonical 值；Android 把 `(commandControlVersion, controlVersion]` 内仍 pending 的命令全部结束，
重新请求 status。Windows 丢弃这些更高版本命令，不得把它们执行到旧歌曲上。

如果事务超过默认 15000ms 仍未结算，服务端使用最后持久化的实际状态产生
failed/execution_timeout；controlVersion 和 appliedControlVersion 均不伪造推进。切歌类 timeout 同样
触发 dependency_failed 连锁终止。
...
failed/dependency_failed。Windows 不执行它们；Android 清除对应 pending，重新读取 status 后再允许
用户操作。普通 seek 失败不会自动把不依赖歌曲变化的后续命令算成 dependency_failed。
...
- authority 断线、连接被替换或服务端重启时，所有 pending 命令进入
  failed/execution_unknown，服务端和 Windows 都不得自动重投；
...
- 切歌失败时，把 failed command 到最新 controlVersion 之间仍 pending 的命令全部结束为
  dependency_failed，并立即刷新 status；
...
- 处理远程事务 deadline 和 execution_timeout；
...
- [x] 用户确认远程命令默认 15000ms 可配置超时，超时使用 execution_timeout。
...
- [x] 固定 dependency_failed、execution_timeout、execution_unknown 和 source-only stale correction。
- [x] 编写完整 Flutter r8→r10 迁移说明。
- [x] 不修改 manifest、fixtures 或 Dart。
- [x] 完成后暂停，由用户把契约同步给服务端。
...
- [ ] 服务端实现 acceptedAtMs/deadlineAtMs 和默认 15000ms execution_timeout。
- [ ] 服务端实现切歌类失败后的 dependency_failed 连锁终止。
- [ ] 服务端实现断线/重启 execution_unknown，删除 pending 自动重投路径。
...
- [ ] dependency_failed 清理失败版本到最新 controlVersion 的 pending 区间并刷新 status。
...
- [ ] 切歌 47 failed、48/49 pending 时，48/49 原子进入 failed/dependency_failed。
- [ ] 默认 15000ms 到期进入 failed/execution_timeout，广播最后 actual snapshot。
...
- [ ] dependency_failed 后清理后续 pending 并请求 status，不自动重发。
...
| 切歌失败 + 后续命令 | 快速发送切歌、pause、seek，切歌失败 | 后续命令全部 dependency_failed；Windows 不执行到旧歌；手机刷新后再操作 |
| 执行超时 | 阻止 Windows 返回 terminal 超过 15 秒 | 服务端 execution_timeout；手机收到最后 actual 状态；命令不永久 pending |
...
- pending 默认 15000ms 必须 terminal；authority 断线和服务重启不得自动重发旧命令。
...
缓解：默认 15000ms execution_timeout；断线和重启统一 execution_unknown，所有结果不明的旧命令
进入 terminal failed 且禁止重投。
