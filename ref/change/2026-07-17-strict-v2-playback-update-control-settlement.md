# Strict-v2 `2.4.0` r11 控制结算与本地优先计划

## 当前状态

状态：**阶段 0—1 已完成并完成服务端/Flutter 双方评审：strict-v2 `2.4.0` r11 契约、ADR-0022、
...
- 不修改 DevicePlaybackState、实际歌曲、状态或位置；
- dependency cascade 原子提交后按 commandControlVersion 从小到大逐条发送；
- 所有 settled 必须先验证 pending、原子写入 failed/errorCode 并持久化成功，才能 emit；
- 发送给全部合法 Context recipients；最初接收 transaction 的 authority Windows Socket 仍在线时必须
  接收，断线后的新物理连接不补收历史 settled；
...
Windows 收到 settled 后按同一事务键使 audio execution lease 失效、从执行队列删除事务，并禁止迟到
callback 切歌、发送 committed 或修改 Context/Queue/系统媒体状态。settled 只取消事务，不直接修改
实际播放状态。
...
5. 本 r11 不增加 `localUser + failed` wire shape；本地失败只解除屏障并使用 passive 实际状态或本地 UI
...
- [ ] settled 必须先持久化 terminal，再发送给 controller 和仍持有该 transaction 的在线 authority
  Windows；新物理连接不补收历史 settled。
...
- [ ] 断线重连后不恢复或重发旧远程事务。
- [ ] 收到 settled 后按事务键使 lease 失效、删除队列项并隔离迟到 callback。
...
- [ ] 收到 dependency_failed/execution_unknown settled 后使对应 lease 失效、删除队列项，迟到 callback
  不切歌、不 committed、不修改 Context/Queue/系统媒体状态。
...
- Windows 默认 executionTimeoutMs=15000，服务端 watchdog 固定加 2000ms；authority 断线和服务重启
  不得自动重发旧命令。
- Windows 未证明迟到执行可被阻止时不得发送 execution_timeout，部署不得标为完整 r11 Core ready。
...
execution_timeout，服务端 watchdog 只能 unknown，部署保持 r11 Core not ready。
