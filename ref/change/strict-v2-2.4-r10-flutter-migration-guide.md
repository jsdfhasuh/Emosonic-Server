...
- server-routed command 必须携带 `executionTimeoutMs`，默认 15000ms；
...
- remoteCommand failed、server-only `playback.control.settled` 或 localUser supersede 都会结束 pending。
...
Windows 在 47 失败后必须丢弃本地的 48、49，不能把 pause/seek 执行到旧歌曲。服务端先广播 Windows
对 47 的 remote failed playback.update，然后按版本逐条发送：

```text
playback.control.settled(commandControlVersion=48, dependency_failed, dependsOn=47)
playback.control.settled(commandControlVersion=49, dependency_failed, dependsOn=47)
```

settled 使用 `requestingClientId` 表示最初发送各命令的 controller，不使用 sourceClientId，也不带
clientSeq/deviceSessionId。Android 按每条具体 commandControlVersion 结束 pending，全部结算后合并请求
一次最新 status，在水合完成前禁止自动重发。
...
### 10.2 Windows 15 秒执行租约

每条第 6.7 节普通 server-routed `queue.playItem` / `player.*` 远程控制命令携带；Handoff commit 继续使用
其独立 effective-at/complete 超时规则：
...
Windows 收到命令并放入本地执行队列后，用该字段启动 audio execution lease，不在客户端写另一套固定
值。15 秒内成功发送 committed；确认实际失败发送对应 failed。

到达 15 秒时，只有 Windows 能保证：

- 对应 lease 已失效；
- 仍在加载的音频任务已停止、取消或隔离；
- 迟到 Future/callback 不能切歌；
- 迟到 callback 不能发送 committed；
- 不能修改 Context、Queue projection 或系统媒体状态；
- 更高版本命令仍能正常执行；

才能发送 remoteCommand failed `errorCode:"execution_timeout"`。Android 收到的是正常 Windows
playback.update，可以显示“执行超时，已恢复实际状态”，刷新 status，不自动重发。

如果 Windows 音频后端做不到上述保证，不得发送 execution_timeout，也不能把硬超时标为已完成。

### 10.3 服务端 17 秒 watchdog

服务端在命令 accepted/routed 后启动：

```text
watchdog = executionTimeoutMs + 2000ms
默认 = 17000ms
```

到期仍未收到 Windows committed/failed feedback 时，服务端只能把事务结算为 execution_unknown，并
发送 server-only playback.control.settled。服务端不能生成 failed playback.update、不能复用 Windows
clientSeq，也不能声称电脑已经安全取消。

execution_unknown 不证明根命令实际失败，因此不能自动把更高 pending 改成 dependency_failed；每条
事务按自己的 feedback、watchdog 或连接失效独立结算。

### 10.4 断线或重启结果不明

authority 断线、连接被替换、服务端 graceful shutdown 或重启恢复时，服务端把每个尚未结束的事务写为
failed/execution_unknown。服务端仍在线且 controller Socket 有效时，为每个事务
分别发送 `playback.control.settled`；完整服务端重启后不补发历史事件，客户端依靠新 nonce 清理旧
pending。
...
Windows 确认的实际执行失败通过 remoteCommand failed playback.update：
...
服务端主动结束但没有新的 Windows feedback，通过 playback.control.settled：
...
| `dependency_failed` | 前一条切歌失败，后续操作已取消 | 按具体 settled version 清除 pending，合并刷新 status |
...
- 使用命令携带的 executionTimeoutMs 建立 execution lease；
- 音频层真正完成后发送 committed；
- 音频层失败后发送 failed 和实际旧状态；
- lease 超时且已安全失效时发送 execution_timeout；
- 迟到 Future/callback 不得切歌、发送 committed 或修改任何播放投影；
- 切歌类失败后丢弃所有更高版本未执行命令；
...
```text
pending_execution -> dependency_failed | execution_unknown
```
...
- playback.control.settled：按具体 commandControlVersion 结束 dependency_failed/execution_unknown；
...
第一次收到写 terminal；相同内容重复忽略，不重复提示或刷新；同一主键不同 status/errorCode 记录协议
冲突并请求 status，不覆盖第一次 terminal。旧 settled 可补齐旧 pending，但不能回滚更新版本的实际
播放状态。客户端不能仅依赖消息到达顺序。
...
- ACK 后立即显示音频执行成功；
- 服务端生成 remote failed playback.update 或复用 Windows clientSeq；
- 根据 `(failedVersion, controlVersion]` 自行猜测 dependency_failed 区间；
- 重连后自动恢复或重放未确认远程命令；
- 把迟到旧状态写入全局播放 UI；
...
- [ ] 新增 server-only playback.control.settled 闭合解析，要求 requestingClientId，禁止 sourceClientId。
- [ ] status deviceStates 要求 appliedControlVersion。
- [ ] 保存 controlVersion 和 appliedControlVersion，不用一个值代替两个含义。
- [ ] 区分四个 Windows failed errorCode 与两个 server settled errorCode。
...
- [ ] 使用 server-routed executionTimeoutMs 建立 audio execution lease。
- [ ] lease 失效后停止、取消或隔离迟到音频任务。
- [ ] 自动化证明迟到 callback 不切歌、不 committed、不修改播放投影。
...
- [ ] dependency_failed 按每条 settled 的具体 commandControlVersion 清理并合并刷新。
- [ ] settled 重复幂等、冲突 fail-closed、旧版本不回滚 applied state。
...
7. 切歌 47 failed、48/49 pending 时，服务端按 48、49 发送 settled，Windows 不执行，Android 精确
   清除为 dependency_failed。
8. command 携带 executionTimeoutMs=15000，Windows 安全失效 lease 后发送 execution_timeout。
9. 音频加载第 17 秒返回时不能切歌、committed 或覆盖新状态。
10. 服务端 17000ms watchdog 无 terminal 时发送 execution_unknown settled，不生成 playback.update。
11. pending 时断开 Windows 或重启服务端，得到 execution_unknown，重连不自动重发。
12. settled 相同主键重复忽略；同键不同 terminal 记录协议冲突并刷新，不覆盖首次结果。
13. lastApplied=48 后发送 applied=47 passive，Android 收不到旧状态，Windows 收到当前 source-only
    correction。
14. Windows localUser observed 46、服务端 canonical 47 时获得 48，并 supersede pending 47。
15. 同 intentId 相同内容重试不增加版本，不同内容 conflict。
16. Socket reconnect 后重新 ensure/list/subscribe/status，旧 nonce 和旧 pending 均不生效。
17. Windows 无法隔离迟到执行时不发送 execution_timeout，部署保持 Core not ready。
18. 自然播完自动下一首不使用 localUser，也不覆盖 pending remote。
...
- dependency_failed/execution_unknown 只由 playback.control.settled 表达，不冒充 Windows feedback；
- 没有 ACK 即成功、永久 pending、断线重放或旧状态回滚；
- 切歌失败不会把后续命令执行到旧歌曲；
- timeout、unknown 和 dependency failure 均有明确 UI 和恢复路径；
- Windows 已证明超时 lease 失效后迟到任务不能切歌或发送 committed；
