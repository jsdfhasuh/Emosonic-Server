...
本 r10 继续使用尚未发布的 strict-v2 `2.4.0` 单一 shape：待机 Context、启动 ensure 和 prepare
规则保持不变，同时把远程命令执行结算与 Windows 本地人工控制统一收敛到条件闭合的
`playback.update`。本轮删除未发布的 `player.authorityIntent`，不保留两套本地控制通道，也不保留
旧的非空 Context-only snapshot 或兼容分支。只有本文全部 conformance tests 和真实双客户端联调
通过后才可标记 ready。后续 wire shape 变化必须同步更新 protocolVersion 和本文，不得静默漂移。
...
- `queue.playItem`、`player.next`、`player.prev` 等切换实际歌曲的远程命令失败时，服务端必须把同一
  ...
  `dependency_failed`；Windows 必须丢弃这些后续命令，controller 刷新实际状态后才能重新发送；
- 每个 pending 远程命令必须有服务端执行期限，默认 15000ms 且允许部署调整。超时使用
  `execution_timeout` 失败并广播最后已保存的实际状态；authority 断线或服务端重启造成结果不明时
  使用 `execution_unknown` 失败，旧命令不得在重连后自动重发；
...
执行屏障并通过 passive 实际状态或本地 UI 错误完成恢复；本 r10 不定义 localUser failed wire shape。
...
每个 pending transaction 必须在持久化记录中保存 `acceptedAtMs` 与 `deadlineAtMs`。默认执行期限为
15000ms，部署可以调整，但该值是服务端配置，不增加 registration capability 或业务 payload 字段。
期限从命令和 pending transaction 原子提交、并可靠加入 authority 当前 Socket 发送路径后开始；
committed、failed、superseded、authority 断线或服务端 shutdown/recovery 都必须取消对应 timer。
...
5. 广播原 command 的 failed canonical update 和对账后的 status/queue state。原 failed update 中的
   `controlVersion` 是当前最高 canonical 值，因此 controller 必须把
   `(commandControlVersion, controlVersion]` 内仍 pending 的命令视为 `dependency_failed` 并刷新 status。

Windows 必须按 controlVersion 串行执行。切歌类 command 失败后，它必须丢弃本地队列中所有更高版本的
未完成远程命令，不得把 pause、seek、play 或下一次切歌悄悄执行到旧歌曲上。

pending transaction 到达 deadline 时，服务端必须自行结算为 failed、使用
`errorCode:"execution_timeout"`，并用持久化的 lastApplied snapshot 生成无 requestId canonical failed
update；不得等待 authority 再补 ACK。若超时 command 属于上述切歌类动作，同时执行 dependency failure
规则。服务端生成的 timeout confirmation 复用已保存 actual snapshot 的 clientSeq，不伪造新的设备
反馈序号。
...
command 是 `queue.playItem`、`player.next` 或 `player.prev`，则该区间内所有更高 pending remote 已按
第 5.2.2 节进入 `dependency_failed`；controller 不等待这些命令逐条产生新的设备 clientSeq，而是立即
结束对应 pending UI、请求最新 status，并只在水合完成后重新发送控制。

authority 断线、被新连接替换、服务端 graceful shutdown 或重启恢复时，所有尚未 terminal 的远程事务
必须进入 failed，`errorCode:"execution_unknown"`。服务端仍在线时应使用最后保存的 actual snapshot
通知 Context recipients；服务端重启导致全部 Socket 断开时，客户端按重连流程清除本地 pending 并以
新 status 为准。任何情况下都不得在 authority reconnect/ensure 后自动重发结果不明的旧 command。
...
**REQ-003 — 主版本兼容**
当服务端声称实现本 r10 契约时，`protocolVersion` 必须是 major `2`、minor `>=4`。服务端不得按客户端
...
按 lastApplied 对账实际状态。Windows 必须丢弃这些后续命令；controller 必须结束对应 pending 并刷新
status 后再操作。

**REQ-040 — Remote execution deadline**
当远程事务在服务端配置的执行期限内未收到合法 terminal 结果时，服务端必须以默认 15000ms 的期限将
其结算为 failed/execution_timeout，使用最后持久化的实际状态生成 canonical failed update，且不得推进
applied cursor 或重新分配 controlVersion。部署可以调整期限，但不得通过 capability 或临时 wire 字段
改变客户端 shape。

**REQ-041 — Unknown execution is never replayed**
当 authority 断线、连接被替换或服务端重启使 pending command 的执行结果无法证明时，服务端必须把
相关事务结算为 failed/execution_unknown。authority reconnect/ensure 后不得自动重投；Flutter 必须清除
旧 pending、重新 list/subscribe/status，并只根据新水合状态继续控制。
...
4. 默认资源上限：每 IP 同时 10 条未认证连接、每用户 20 条已认证连接；每连接每分钟 120 个
   ...
   远程执行期限默认 15000ms，允许部署调整；调整必须保持有界且不能关闭 terminal settlement。
...
- authority 断线、连接被替换、graceful shutdown 和重启恢复时，pending remote control 一律结算为
  failed，`errorCode:"execution_unknown"`。服务端不得在 authority 重新 ensure/status 后按原版本重投；
  客户端必须清除旧 pending 并从新 status 恢复，避免重复音频执行。
...
dedupe 和 error conformance 全部通过后，才可用 major `2`、minor `>=4` 的 `protocolVersion` 接受
...
31. 切歌命令 47 失败而 48、49 仍 pending 时，47 使用原失败码，48、49 原子进入
   failed/dependency_failed；Windows 不执行 48、49，failed push 的 controlVersion 保持 49，手机清除
   对应 pending 并重新请求 status；
...
40. 远程事务超过默认 15000ms 未结算时进入 failed/execution_timeout，广播最后 actual snapshot；切歌类
   timeout 同时终止更高 pending，controlVersion 和 appliedControlVersion 均不伪造推进；
41. authority 断线或服务端重启时所有 pending 进入 failed/execution_unknown，重连 status 恢复
   canonical N 和 per-device applied M，不自动重投任何旧 command；
42. 自然播完自动下一首不使用 origin localUser，不获得本地人工 supersede 权限；
43. Android 与 Windows 各一台真实 Flutter 客户端联调，记录可复现日志。
...
本文从 Flutter strict-v2 vertical slice 提取；r10 继续使用 `2.4.0`，保留待机 Context、启动 ensure、
