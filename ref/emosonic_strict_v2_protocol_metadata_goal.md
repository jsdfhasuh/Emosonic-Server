...
commit，并为每条 Socket.IO 连接提供不可复用的 nonce 与当前 connection epoch。
...
3. 当前服务端镜像所对应的源码提交。
4. 当前 Socket.IO 连接的随机证明；
5. 该动态连接证明的 epoch。

Flutter lab 使用由部署 CI 生成或提供的预期值校验三项静态值，不能从 Flutter 自身 commit、示例值或测试 fixture 推测服务端身份。
...
3. strictV2 对象固定包含上述五项；前四项是非空字符串，connectionEpoch 是固定为 1 的整数。
4. client 保持当前序列化和字段行为；本 Goal 不借此承诺 client 的全量字段集合。
5. schemaHash 仅代表第 4 节定义的注册握手描述符。
6. connectionNonce 由服务端在每条 Socket.IO 连接建立时生成，断开并重新连接后必须变化；它不得来自客户端、fixture 或部署 manifest。

---

## 3. 三项静态值与连接证据的语义和 Flutter lab 校验

| 字段 | 服务端含义 | Flutter lab 的校验方式 |
| --- | --- | --- |
| protocolVersion | 注册握手 profile 的语义版本，例如 2.1.0 | 必须属于 lab 支持的版本集合 |
...
| connectionNonce | 当前 Socket.IO 连接的服务器生成随机字符串 | 必须为非空字符串；重连后必须与旧连接不同 |
| connectionEpoch | 动态连接证据语义的 epoch | 当前必须精确等于整数 1 |

必须区分协议兼容与部署溯源：

- protocolVersion 和 schemaHash 用于判断 lab 是否理解该注册握手 profile；
- serverBuildCommit 用于确认 lab 连到预期部署，不作为普通客户端的通用协议兼容门槛；
- lab 的静态预期三项值必须来自受信任的部署 manifest、CI 输出或测试环境配置；
- lab 不得先读取 ACK 再把 ACK 自己当作预期值；
- lab 不得使用 Flutter commit、fixture 或示例 hash 作为服务端 build identity。
- lab 必须直接校验 ACK 的 connectionNonce 非空，且重连后变化；它不应尝试在连接前预设 nonce。
- lab 必须直接校验 ACK 的 connectionEpoch=1。
...
- strictV2 内 protocolVersion、schemaHash、serverBuildCommit、connectionNonce 和 connectionEpoch 的形状与 required 约束；
- 注册失败 system.error 的基本信封和 code/message 形状；
- 注册相关消息的 nullable 字段、enum 和必要数值约束。

其中，payload.client 只要求稳定的注册核心字段，并允许当前服务端已有的额外 client 字段；payload.strictV2 必须禁止未知字段。serverBuildCommit 必须匹配 unknown 或 ^[0-9a-f]{40}$，schemaHash 必须匹配 ^[0-9a-f]{64}$，connectionNonce 必须为非空字符串，connectionEpoch 必须为整数常量 1。
...
get_strict_v2_registration_metadata(connection_nonce)
~~~
...
- get_strict_v2_metadata() 返回新的三项静态 dict，避免调用者修改缓存对象；
- get_strict_v2_registration_metadata(connection_nonce) 将非空、当前连接的 nonce 与 connectionEpoch=1 加入新的 ACK metadata dict；
...
- strict-v2 成功注册必须返回三项静态元数据与两项动态连接证据；
...
- protocolVersion 能读取，初始值为 2.1.0，缺失或类型错误时明确失败；
...
- registration metadata 保留三项静态值，并包含调用者传入的非空 connectionNonce 与 connectionEpoch=1。
...
- 验证缺失 connectionNonce 或 connectionEpoch 不为 1 的 ACK 不符合描述符；
...
- strictV2 的三项静态值等于 get_strict_v2_metadata()；
- strictV2 有非空 connectionNonce 和 connectionEpoch=1，且不同 Socket.IO 连接的 nonce 不同；
...
- lab 的三项静态预期值来自部署 manifest/CI，而非 Flutter 自身；
- lab 对 connectionNonce 做非空与重连变化校验，并精确校验 connectionEpoch=1；
...
在 strict-v2 device.register ACK 中返回三项静态元数据与两项动态连接证据，并补 WebSocket 兼容测试。
...
- strict-v2 注册 ACK 返回三项静态 metadata 与 connectionNonce、connectionEpoch 动态连接证据；
- connectionNonce 每个 Socket.IO 连接唯一且重连后变化，connectionEpoch=1；
- legacy 注册行为保持兼容；
...
- Flutter lab 可从受信任部署 manifest 逐字校验三项静态值，并校验动态连接证据；
