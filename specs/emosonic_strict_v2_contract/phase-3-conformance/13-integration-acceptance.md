...
本页 1—110 项在 `2026-09-04-r19` 冻结时全部是 contract-defined acceptance；实现与证据状态以
r19 对应的 requirement mapping 为准。
...
3. 只有 major `2`、minor `>=9` 且除可选 schemaHash 外其余必需字段完整的固定注册 shape 可进入本文
   strict Core；低于 `2.9.0`、其他 major、缺少必需字段、非法 serverBuildCommit 或 actionless ACK
...
27. negotiated register 返回低于 `2.9.0`、其他 major，或合规连接上 Core action 返回
...
28. passive playback.update 只更新实际进度/状态/播放速度，controlVersion 和全部 Context cursor 不变；
     ...
     playing authority 至少每秒上报一次。低于 lastApplied 的 stale passive 只返回
     `system.error(code=conflict,payload.action=playback.update,retryable=true)` 与当前四 cursor，数据库、
     clientSeq/cursor 和订阅者均无副作用，不生成 canonical playback.update；Flutter 保留 dirty playback，
     status 对账后用新 clientSeq 重发。只有完整 submission fingerprint 的 canonical 回声可结算 playback
     outbox，Context/Queue snapshot 不能结算；内部 `clientSeq=0` baseline 不输出到 deviceStates；
...
42. `broadcast.start` 只接受 playbackContextId/intentId/participants；旧 queue/index/position/autoPlay
    字段返回 bad_request。source 未结算、没有当前 authority exact pair/physical nonce/epoch 且
    `clientSeq>=1`、applied==control 的 DevicePlaybackState、queue idle 或实际不是
...
82. dependency failure 按直接依赖和 controlVersion 顺序递归生成 dependency_failed；authority
    ...
    sid 去重；replacement requester 不获历史 settlement。watchdog 每轮六项 sweep 共用一个 owner-aware
    数据库连接，非 1040 单项异常不阻断其余项；异常链出现 MariaDB/MySQL 1040 时中止整轮、合并一次
    warning 并按 1/2/4/8/16/30 秒退避，成功一轮后复位；
...
86. Follow source fact 必须来自 authority exact pair 当前 nonce/epoch 的真实 DevicePlaybackState，且
    clientSeq>=1、applied==control；Context/Queue snapshot 与内部 baseline 都不能满足。playing 的两个
...
87. Flutter 在 follow.start 前验证 suspended Context/command lane/lease/overlay 并先持久化 acquiring
    RecoveryRecord；preflight 必须从当前连接代次 status 取得 suspended authority exact pair 的真实
    DevicePlaybackState，并要求 clientSeq>=1、applied==control。服务端用同一门槛返回完整 frozen exact
    pair/cursors/applied baseline。逐字段匹配才进入 audio overlay；baseline mismatch、prewrite 失败、
    ACK 前后 crash 与 active-record 写失败都只走 stop/stopPending cleanup，不触碰或误恢复镜像音频。
