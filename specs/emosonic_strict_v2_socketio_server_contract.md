...
> 范围：PlaybackContext v2 strict-v2。本文定义客户端与服务端应发送、应接受的协议契约；不是旧 `sessionId` 协议的兼容说明，也不授予 production rollout 权限。
...
本 r8 在 `2.2.0` Context discovery 基线上增加 `2.3.0` 设备级远程音量控制，使在线 player
无论是否存在 PlaybackContext 都能按 client/device pair 被精确控制。此前实验性 `2.1.x` 部署不形成兼容性
承诺；只有本文全部 conformance tests 和真实双客户端联调通过后才可标记 ready。r8 冻结后，任何破坏性 wire 变更
必须升级为 `3.x`。
...
- `device.list` 只发现设备；从目标 `authorityClientId` / `authorityDeviceSessionId` 到 active Context
  ...
  缓存失效；
- 本契约要求 `protocolVersion` 的 major 为 `2` 且 minor 至少为 `2`；`2.0.x` / `2.1.x` 不具备本文规定的 Context discovery Core，Flutter 必须 fail-closed。`device.setVolume` 与 capability 扩展要求 minor 至少为 `3`；`schemaHash` 和 `serverBuildCommit` 是部署观测值，不是要求客户端每次打包固定的 pin；破坏性变更必须升为 `3.x`；
...
- `capabilities`：必须有基础 9 个 bool 字段；`remoteVolumeControl` 是 `2.3.0` 可选第 10 个 bool；不得出现其他字段；
...
`2.3.0` 兼容协商采用闭合双形状：首次 probe 可以继续发送基础 9 字段并收到基础 9 字段 ACK；
客户端观察到服务端 minor `>=3` 后，在 negotiated reconnect 中增加 `remoteVolumeControl`。服务端
必须按请求形状返回 9 或 10 字段 `negotiatedCapabilities`，不得向 9 字段客户端主动增加字段。
...
| `strictV2.protocolVersion` | 是 | 解析 numeric major/minor；major 必须为 `2` 且 minor 必须 `>=2`，不得用字符串字典序比较；其他版本 fail-closed |
...
| `negotiatedCapabilities` | 是 | 与请求 capability 形状一致的完整 9 或 10 个 bool 字段；后续 capability gate 的唯一依据 |
...
注册握手描述符必须覆盖 ACK 的 `payload.action`、`clientId`、`deviceSessionId`、完整基础 9 个 bool 与可选 `remoteVolumeControl`
...
| `playback.context.create` | 同 `requestId` 的 direct `playback.context.create` response，payload 为第 6.2 节完整 snapshot |
...
| `device.volume.update`、`playback.update`、`playback.ready`、`playback.handoff.complete` | event/state-confirmed；服务端不回 ACK，按第 4.5、5.2、5.4、6.0、6.6、6.9 节状态推进 |
...
List/Create 不允许用简略 ACK 代替 direct response。
...
6. Socket 断开必须清除该 sid 的临时订阅和 client→sid 映射，但不得删除持久化 Context。
   ...
   deviceSessionId 的连接不得匹配旧 binding。
...
6. 返回 0 个 Context 时客户端显示无可用 playback scope；返回 1 个时可自动 subscribe/status；
...
authority 发送命令，也不得修改任何 cursor。create、close、handoff authority switch 与普通
...
| Action / type | payload | 服务端动作与响应 |
| --- | --- | --- |
| `playback.handoff.start` / `command` | `playbackContextId:R`、`targetClientId:R`、`baseControlVersion:R int>=0` | 原子创建 handoff 后返回 ACK，并给 target 发第 6.8 节 prepare；不同 Socket 间不承诺到达顺序。ACK payload **只允许且必须**有 `action`、`handoffId`、`prepareId`、`status:"preparing"`、`controlVersion`。 |
...
1. `playback.context.create` 创建 active Context：为新 Context 的 authority/device pair 发送；
...
### 6.2 `playback.context.create`：直接创建回执
...
创建时 authority 必须是请求 player，Context 同时持久化其 `authorityDeviceSessionId`。该字段只在
第 6.1.1 节的 Context binding 中公开，不属于 create/status snapshot。创建 Socket 自动加入订阅；
断线后仍按第 4.4 节清除临时订阅。
...
**REQ-003 — 主版本兼容**  
当服务端声称实现本 r8 契约时，`protocolVersion` 必须是 major `2`、minor `>=3`；基础 Context
Core 仍与 minor `2` 兼容，设备级远程音量只在 minor `>=3` 协商，并保持已有
strict-v2 wire contract 向后兼容；`2.0.x` / `2.1.x` 不得声称具备本文 Core。当存在破坏性变更时，
服务端必须升主版本。
...
当 create、close、handoff complete 或其他 mutation 改变某个 authority/device pair 的 active
...
当服务端接受普通 player control 时，服务端必须在 authority/device pair 级别与 create、close、
...
- active Context 与 closed tombstone 必须持久化。重启恢复时保留
  ...
  nonce、connectionEpoch、request cache 和 subscription，等待客户端重新注册，并用新的 requestId
  重新 list/subscribe/status。
...
Core profile 等于握手/注册、Context discovery/binding invalidation、PlaybackContext、Queue 和
Player Control，缺一不可；服务端只有在 `playback.context.list`、
`playback.context.bindings.changed` 及其他 Core action 的 request/response/event、cursor、routing、
dedupe 和 error conformance 全部通过后，才可用 `protocolVersion>=2.2.0` 接受
...
`remoteVolumeControl` 是 `2.3.0` Core extension，不设置独立部署开关。服务端只有在设备级请求、
目标路由、在线状态、event confirmation、9/10 capability 兼容与断线清理全部通过 conformance 后，
才能向请求该扩展的连接协商 true；基础 9 字段连接仍可使用其余 `2.2+` Core。
...
`canSetVolume` 只表示 player 能执行本地音量设置；`remoteVolumeControl` 表示连接理解 `2.3.0`
...
3. major `2` 且 minor `>=2` 的兼容部署只更新 hash/commit 观测；`2.0.x` / `2.1.x`、`3.x`、缺字段、非法 hash/commit、actionless ACK 均关闭本文 strict Core；
...
5. `device.list` 不含 Context binding；`playback.context.list` 的 direct response、controller/user
   ...
   人工或自动选择；
6. 基础 9 字段 probe 与扩展 10 字段 negotiated reconnect 都能注册；`device.setVolume` 可控制在线
...
7. list -> subscribe -> status -> conditional queue sync / player control 的唯一结算、完整 canonical
   queue/playback/cursors、canonical push 和 cursor conflict；并发 create/handoff 使同 pair 变为多
   Context 时，在失效事件到达前发送的控制也必须由 pair-level 原子检查拒绝，且 cursor 不变；
8. create 同 ID 新 requestId 重试、同 requestId 60 秒缓存重放、内容冲突、断线清 subscription 和重连水合；
9. Cursor 矩阵每一行的递增、不递增、旧值拒绝、等值去重、必需 clientSeq 和四个 event-confirmed action 的精确单 Socket 重放；
...
13. create、close、handoff complete 分别向所有同用户 strict controller（包括非 subscriber）发送
...
19. Android 选择 Windows 后发现唯一 Context，status 水合队列/索引/播放状态/cursors，`player.pause`
   在 Windows 执行，`queue.playItem` 切换正确歌曲；Windows 以同 client/device pair 重连后 Android
   使用新 requestId 重新 list/subscribe/status 并恢复控制，全程不使用 session fallback；
20. negotiated register 返回 `2.0.x` / `2.1.x`，或 major `2`、minor `>=2` 的连接上 list 返回
   `not_supported` / `capability_required` 时，Flutter fail-closed、清理远控状态且绝不 session fallback；
20. Android 与 Windows 各一台真实 Flutter 客户端联调，记录可复现日志。
...
本文最初从以下 Flutter 代码位置提取；r7 新增 Context discovery/binding invalidation，r8 新增
设备级远程音量。服务端和 Flutter 实现及 fixtures 都必须更新，并通过 conformance tests 保持一致：
