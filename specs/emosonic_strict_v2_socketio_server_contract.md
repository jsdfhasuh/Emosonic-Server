...
本 r7 是 `2.2.0` 的冻结候选基线，并补齐从目标 authority/device 发现 PlaybackContext、binding
失效通知与多 Context fail-closed 的 strict Socket 契约。此前实验性 `2.1.x` 部署不形成兼容性
承诺；只有本文全部 conformance tests 和真实双客户端联调通过后才可标记 ready。r7 冻结后，任何破坏性 wire 变更
必须升级为 `3.x`。
...
- `device.list` 只发现设备；从目标 `authorityClientId` / `authorityDeviceSessionId` 到 active Context
  ...
  缓存失效；
- 本契约要求 `protocolVersion` 的 major 为 `2` 且 minor 至少为 `2`；`2.0.x` / `2.1.x` 不具备本文规定的 Context discovery Core，Flutter 必须 fail-closed。`schemaHash` 和 `serverBuildCommit` 是部署观测值，不是要求客户端每次打包固定的 pin；破坏性变更必须升为 `3.x`；
...
| `playback.update`、`playback.ready`、`playback.handoff.complete` | event/state-confirmed；服务端不回 ACK，按第 4.5、5.4、6.6、6.9 节状态推进 |
...
`targetClientId` 也禁止，唯一例外是第 5.4 节的 `playback.handoff.start`。
...
**REQ-003 — 主版本兼容**  
当服务端声称实现本 r7 契约时，`protocolVersion` 必须是 major `2`、minor `>=2`，并保持已有
...
`canSetVolume` 仍是设备能力描述和未来兼容位，但在 2.2.0 中没有对应 strict action；它为 true
...
5. `device.list` 不含 Context binding；`playback.context.list` 的 direct response、controller/user
   ...
   人工或自动选择；
6. list -> subscribe -> status -> conditional queue sync / player control 的唯一结算、完整 canonical
...
7. create 同 ID 新 requestId 重试、同 requestId 60 秒缓存重放、内容冲突、断线清 subscription 和重连水合；
8. Cursor 矩阵每一行的递增、不递增、旧值拒绝、等值去重、必需 clientSeq 和三个 event-confirmed action 的精确单 Socket 重放；
9. authority clientId/deviceSessionId 双重绑定、同 pair 重连可重新发现、相同 clientId 不同
   deviceSessionId 不匹配旧 Context、旧 sid 强制断开，以及永久离线后 close + 新 Context 恢复；
10. Handoff 的 prepare -> ready -> effective-at commit -> complete -> authority binding 原子迁移 -> release，
   以及 8 秒/5 秒超时和 source/target 断线；
11. Handoff complete 后旧 device pair 的 list 不再返回该 Context、新 pair 返回同一 Context ID；close
   后 list 立即排除旧 ID，缓存旧 ID 的 status/control 返回 `context_closed`；
12. create、close、handoff complete 分别向所有同用户 strict controller（包括非 subscriber）发送
...
13. Follow、Handoff 与 Broadcast 仅在 negotiated capability 为 true 且 profile ready 时测试，包括 owner/authority 权限、
   participant 禁止控制、Handoff errorCode 和 terminal 幂等；
14. Broadcast 初始 participantStates 省略 paired feedback 字段，首个 feedback 后两字段同时出现；
15. transport message-too-big 断开、business limit bad_request、非法 requestId/action 断开，以及未知字段/null/rate 限额；
16. nonce CSPRNG/128-bit 熵，以及 queueSongIds 在序列化、持久化和重启后的顺序保持；
17. 单 worker 启动保护、Context/tombstone 重启恢复，以及 Handoff/Follow/Broadcast 显式终止；
18. Android 选择 Windows 后发现唯一 Context，status 水合队列/索引/播放状态/cursors，`player.pause`
...
19. negotiated register 返回 `2.0.x` / `2.1.x`，或 major `2`、minor `>=2` 的连接上 list 返回
...
本文最初从以下 Flutter 代码位置提取；r7 新增的 Context discovery/binding invalidation 契约属于后续 normative
补充。服务端和 Flutter 实现及 fixtures 都必须更新，并通过 conformance tests 保持一致：
