...
服务端广播的 `payload.updatedAt` 是服务端 epoch seconds。Flutter 端应根据消息顶层 `timestamp` 估算服务端时间偏移：
...
```dart
final serverNowMs =
    DateTime.now().millisecondsSinceEpoch + followState.serverClockOffsetMs;
final targetPositionMs =
    sourcePositionMs + (serverNowMs - sourceUpdatedAtSeconds * 1000);
```
