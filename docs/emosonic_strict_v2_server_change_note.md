...
- 基础 9 个 capability 必须全部存在且均为 JSON boolean；`2.3.0` negotiated reconnect 可增加
  可选 boolean `remoteVolumeControl`，probe 继续允许基础 9 字段；
...
- 所有响应使用原请求的 `requestId`。显式 `device.list` 和
  `playback.context.list` 响应也会回显其 `requestId`。
- strict payload 不会带 `sessionId`，业务 envelope 不会带顶层 target。客户端请求中的 payload
  target 只允许 Handoff target 和 `device.setVolume` 的精确 client/device pair；服务端 push 不回显
  target 字段，按实际 Socket 收件人路由。

### 4.1 r7 Context discovery 与 binding 失效

已注册且具有 controller 角色的 strict-v2 客户端，应使用
`playback.context.list`，按目标 `authorityClientId` 和
`authorityDeviceSessionId` 精确查询当前用户的 active Context。服务端用同一 `requestId`
直接返回按 `playbackContextId` 排序的 binding 列表；空列表是成功结果，不会自动建立订阅。

Context create、close 或 Handoff 改变 authority binding 后，同用户 strict controller 会收到无
`requestId` 的 `playback.context.bindings.changed`。收到事件后应暂停该 authority/device pair 的
控制并重新执行 list、subscribe、status。现有 Web 控制台仍使用
`GET /emo/web-context-bindings`，其 JSON surface 未改变。

### 4.2 r8 在线设备远程音量

`2.3.0` 新增设备级 `device.setVolume`。controller 使用目标 `targetClientId`、
`targetDeviceSessionId` 和 `volume:0..100` 精确控制当前在线 player，不要求目标存在
PlaybackContext。请求者必须协商 `remoteVolumeControl:true`；目标必须同时协商
`remoteVolumeControl:true` 与 `canSetVolume:true`。

成功 ACK 只表示命令已路由。目标应用后使用 event-confirmed `device.volume.update` 上报实际值；
服务端把该值作为在线瞬态状态提供给扩展 controller 和 `device.list.volumeState`。设备断线或连接
被替换时状态清除，不保存离线命令，也不修改 PlaybackContext。
