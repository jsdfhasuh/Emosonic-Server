...
本轮不是创建 r19，也不升级 `protocolVersion`。当前 strict-v2 `2.8.0/r18` 尚未正式发布，本轮属于 r18 在冻结前的最终闭合、定稿和一致性修正。

本轮完成并通过最终一致性审计后，权威契约应标记为：

```text
文档状态：Approved r18 authoritative contract
文档修订：2026-08-01-r18
协议版本：2.8.0
冻结状态：r18 Frozen implementation baseline
```

冻结后的纪律：

- 服务端或 Flutter 与 r18 不一致时，默认修改实现适配 r18。
- 纯错字、链接、示例或不改变行为的文字澄清可作为 r18 errata。
- 冻结后新增 action、字段、状态、错误码、持久化义务或改变客户端行为，必须进入下一修订 r19，并按届时的 wire 变化决定新的 `protocolVersion`。
...
- source facts 超过 3 秒不新鲜时，follower 暂停并停止位置外推。
...
11. `playback.control.settled` 的收件人遵守 D17。
...
6. source Context close 时终止 relationship并推 closed。
...
- strict Follow 镜像不会污染任何正常 Context。
...
2. 入口摘要增加 Follow 音频语义、control settlement、Handoff 完整 fact、restore freeze 与 queue boundary。
3. 全部 19 个分卷页首同步为 `2026-08-01-r18` / `2.8.0`。
4. 每项新/修改 REQ 建立目标测试映射，不提前标 Verified。
5. 明确调试 capability 默认 true 不等于 production rollout，但不是实现缺陷。
6. 最终 readiness 证据继续要求 Android + Windows 双端真机。
7. 最终一致性审计通过后，将 r18 标记为 `Frozen implementation baseline`。
8. 明确冻结后：实现偏差改代码；新行为进入 r19；纯文字问题使用 r18 errata。
...
3. 搜索所有 `restorePending`、`restoringOriginalContext`、排队、controlVersion queue：不得残留“服务端允许命令、Flutter 排队执行”的旧 normative 规则。
4. 搜索 Follow：不得再把 strict Follow 定义成只读观察；不得要求 strict Follow mirror 发送普通 `playback.update`。
5. 搜索 Handoff：不得残留仅 `targetClientId` 的 start shape；不得残留仅 `positionMs` 的 complete shape。
6. 搜索 `playback.control.settled` 和 `executionTimeoutMs`：必须在 action surface、字段表、结算矩阵、幂等、EARS、acceptance 中完整出现。
7. 搜索 queue 边界：prev/next/natural-end 在所有章节中不得互相矛盾。
8. 检查 Markdown 链接、章节引用、REQ 引用和 r18 mapping。
9. 运行 `git diff --check`。
10. 允许运行纯文档或静态检查，但契约阶段不得为了让旧代码测试通过而修改代码、validator、fixtures 或客户端。
...
```text
1. spec: close r18 core control settlement
2. spec: define r18 strict follow audio semantics
3. spec: harden r18 handoff execution proof
4. spec: simplify r18 broadcast restore gate
5. spec: finalize and freeze r18 authority baseline
```
...
- Follow、Broadcast、Handoff 的角色与本地覆盖层边界无歧义。
- Handoff start/prepare/commit/complete 字段在所有章节完全一致。
- `restorePending` 不再存在两套相反的命令处理规则。
- `playback.control.settled` 和 `executionTimeoutMs` 不再只存在于实现或历史 ADR。
- r18 mapping 对全部新/改要求有明确实施状态，未实现项没有伪造 Verified。
