...
def _validate_manifest(value: object) -> Dict[str, bool]:
...
@lru_cache(maxsize=1)
def _load_conformance_readiness() -> Dict[str, bool]:
    try:
...
def get_code_conformance_readiness() -> Dict[str, bool]:
    return dict(_load_conformance_readiness())


def is_profile_code_conformance_ready(profile: str) -> bool:
    if profile not in STRICT_V2_PROFILES:
        return False
    return _load_conformance_readiness()[profile]


def validate_conformance_manifest(value: Mapping[str, object]) -> Dict[str, bool]:
    return _validate_manifest(dict(value))
